    ...
    public static class AVProStubsInstaller {
        ...
        static void InstallAVProStubs(bool silent, bool forced) {
            if (Type.GetType("RenderHeads.Media.AVProVideo.MediaPlayer, AVProVideo.Runtime", false) != null) {
                if (!silent) Debug.LogWarning("You have already installed full version of AVPro Video, no need to install stubs.");
                return;
            }
            ...
            if (!silent) Debug.LogError("Cannot find AVPro Stubs, please make sure you have installed VizVid package.");
        }
