...
### プレイリスト/キュー表示順序の反転方法<a name="プレイリスト-キュー表示順序の反転方法"></a>
プレイリスト/キューをデフォルトの降順（下から再生）ではなく、昇順（上から再生）で表示することができます。
1. ヒエラルキータブの検索バーで「Play List Entries」または「t:PooledScrollView」を入力し、UIコントロール内の **Play List Entries** を見つけ、選択します。
2. インスペクターで **Inverse Order** プロパティを見つけ、有効化します。
