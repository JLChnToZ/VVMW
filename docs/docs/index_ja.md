...
> [!NOTE]
> このマニュアルは v1.4.7 から v1.4.8 まで対応しており（新しいバージョンのマニュアルは現在作成中です）、一部の内容は古いバージョンと異なります。
...
### プレイリスト / キュー表示順を反転させる方法<a name="キュー表示順を反転させる方法"></a>
プレイリスト / キューの表示順位をデフォルトの上方向を、下方向に設定します。
1. ヒエラルキーの検索バーで「Play List Entries」か、「t:PooledScrollView」を入力し、UIコントロール内の **Play List Entries** を選択します。
2. インスペクターに **Inverse Order** プロパティを見つけ、有効化します。
...
#### Screen Configurator
VizVidのデフォルトスクリーンを素早く設定するためのコンポーネントです。異なるコアにアサインされたスクリーンを設定したり、スクリーンの簡単な設定オプションを提供します。詳細は [VVMW (Game Object)](#vvmw-game-object) をご参照ください。
...
### VRC Light Volumes
[VRC Light Volumes](https://github.com/REDSIM/VRCLightVolumes) は Light Probes の代替として，アバターや他動的オブジェクトに、更に良いライティングを付けられるアセットです。VizVid は VRC Light Volumes との統合が可能になりました。

2つの統合方法があります：

1. ワンクリックセットアップ：
(VRC Light Volumes 2.0 とその以降、また VizVid 付属したスクリーンが対応対象のみとなりますので、あらかじめご了承ください。)
    Unityヒエラルキーにある VizVid スクリーンオブジェクトを右クリック、`VizVid > Light Volume for Screen` を選択します。
2. 手動セットアップ：[この動画](https://x.com/JLChnToZ/status/1925848509837680684/video/1)の手順にセットアップしてください。「Light Volume TVGI」の代わりに，「Light Volume Adaptor (VizVid)」を使用することができます。
